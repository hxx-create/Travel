<template>
  <div>
    ...
    <!-- /选项卡 -->
    <!-- 表格 -->
    <el-table :data="orders.list" :cell-style="changeCellStyle">
      ...
      <el-table-column fixed="right" label="操作">
        <template v-slot="slot">
          <a class="el-icon-info" href="javascript:void(0)" >详情</a>
          <a class="el-icon-sold-out" href="" v-if="slot.row.status === '待派单'" @click.prevent="toSendOrderHandler(slot.row)">派单</a>
        </template>
      </el-table-column>
    </el-table>
    <!-- /表格结束 -->
...
export default {
  ...
  methods:{
    ...
    // 加载订单信息
    loadData(){
      let url = "http://39.96.84.188:6677/order/queryPage"
    ...
    submitHandler(){
      let url = "http://39.96.84.188:6677/order/sendOrder";
      request({
        url,
        method:"GET",
        params:{
