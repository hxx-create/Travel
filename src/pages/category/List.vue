<template>
    ...
    <!-- 表格 -->
    <el-table :data="categorys" :cell-style="changeCellStyle">
...
export default {
    methods:{
      loadCategoty(){
        ...
        submitHandler(){
          //前端向后台发送请求，完成数据的保存操作
    let url="http://39.96.84.188:6677/category/saveOrUpdate"
    request({
      url,
      method:"POST",
      headers:{
...
request.get(url).then((response)=>{
  ...
  this.categorys = response.data  //把查询结果放置到product中
        ...
        toDeleteHandler(id){
          ...
          //调用后台接口,完成删除操作
          let url = "http://39.96.84.188:6677/category/deleteById?id="+id;
          request.get(url).then((response)=>{
            //刷新数据
            this.loadData();
            //提示结果
